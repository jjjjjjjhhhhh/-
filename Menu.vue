		  ...
		  <el-dialog
			...
			<el-form :model="editForm" :rules="editFormRules" ref="editForm" label-width="100px" class="demo-editForm">
			  ...
			  <el-form-item label="排序号" prop="orderNum" label-width="100px">
					<el-input-number v-model="editForm.orederNum" :min="1" label="排序号">1</el-input-number>
			  </el-form-item>
	...
	methods: {
		...
		submitForm(formName) {
			this.$refs[formName].validate((valid) => {
			 if(valid)
			 {
				this.$axios.post('/sys/menu/' + (this.editForm.id?'updata':'save'), this.editForm).
				then(res =>{
		...
		delHandle(id) {
			...
			this.$axios.post('sys/menu/del' + id).then(res => {
				this.$message({
				         showClose: true,
				         message: '删除成功',
				         type: 'success',
